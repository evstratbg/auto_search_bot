...
def start_search_auto(bot: Bot, update: Update, user_data: dict) -> int:
    ...
    send_params = {
        ...
        'text': 'Поможем найти машину, которая полностью '
                'будет соответствовать вашим требованиям и еще, будет лучшим из тех, '
                'что есть на вторичном рынке. Данный выбор полезен тем, что мы '
                'проанализируем весь вторичный рынок, проверим автомобиль по всем базам '
                'и даже договоримся с продавцом. В связи с этим, данный автомобиль '
